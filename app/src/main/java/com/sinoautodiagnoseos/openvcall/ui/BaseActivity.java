...
public abstract class BaseActivity extends AppCompatActivity {
    ...
    public boolean checkSelfPermission(String permission, int requestCode) {
        ...
        if (Manifest.permission.CAMERA.equals(permission)) {
            ((AppContext) getApplication()).initWorkerThread();
        }
        return true;
    }

    protected RtcEngine rtcEngine() {
        return ((AppContext) getApplication()).getWorkerThread().getRtcEngine();
    }

    protected final WorkerThread worker() {
        return ((AppContext) getApplication()).getWorkerThread();
    }

    protected final EngineConfig config() {
        return ((AppContext) getApplication()).getWorkerThread().getEngineConfig();
    }

    protected final MyEngineEventHandler event() {
        return ((AppContext) getApplication()).getWorkerThread().eventHandler();
    }
