...
import com.sinoautodiagnoseos.net.requestApi.BaseApi;
...
public class NetRequestApi extends BaseApi {
    public static NetRequestApi httpRequestApi;
    public HttpRequestService httpRequestService;

    public NetRequestApi(){
        httpRequestService= NetMethod.getInstance().createApi(HttpRequestService.class);
    }

    public static NetRequestApi getInstance(){
        if (httpRequestApi==null){
            httpRequestApi = new NetRequestApi();
        }
...
//    }
    ...
    /**
     * 智库
     * @param caseCategory  1：技术通报，2：案例，3：汽车资料
     * @param keyword 关键字搜索
     * @param pageIndex
     * @param pageSize
     * @param subscriber
    ...
    public void findcasedetail(int caseId , Subscriber<CaseDetail> subscriber){
        Observable observable = httpRequestService.findcasedetail(caseId)
                .map(new HttpResultFunc<CaseDetail>());
        toSubscribe(observable,subscriber);
    }
     ...
     * @param subscriber
     */
    public void findbrandpage(int pageIndex, int pageSize,
                              Subscriber<Brand> subscriber) {
        Observable observable = httpRequestService.findbrandpage(pageIndex,pageSize)
                .map(new HttpResultFunc<Brand>());
        toSubscribe(observable,subscriber);
    }

    public void getZKCarBrands(Subscriber<ZKCarBrands> subscriber) {
        Observable observable = httpRequestService.findbrandpage()
                .map(new HttpResultFunc<ZKCarBrands>());
        toSubscribe(observable, subscriber);
    }
     ...
     * @param subscriber
     */
    public void findallfault(Subscriber<Fault> subscriber) {
        Observable observable = httpRequestService.findallfault()
                .map(new HttpResultFunc<Fault>());
        toSubscribe(observable,subscriber);
    }
}
